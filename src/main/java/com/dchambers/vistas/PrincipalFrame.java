...
public class PrincipalFrame extends JFrame {
    ...
    public PrincipalFrame(Usuario usuario) {
        ...
        addWindowListener(new java.awt.event.WindowAdapter() {
            ...
            public void windowClosed(java.awt.event.WindowEvent e) {
                RedCliente.getInstancia().removerObservador(PrincipalFrame.this::refrescarPantallaActual);
            }
        });
        mostrarInicio();
    }
    ...
    private JPanel crearMenu() {
        ...
        rol.setForeground(new Color(195, 200, 225));
        ...
        if (usuario.getRol().equals("CAJERO") || usuario.getRol().equals("ADMIN")) {
            menu.add(botonMenu("Crear orden", this::mostrarCrearOrden));
            menu.add(Box.createRigidArea(new Dimension(0, 14)));
            menu.add(botonMenu("Ver órdenes", this::mostrarOrdenes));
            menu.add(Box.createRigidArea(new Dimension(0, 14)));
            menu.add(botonMenu("Generar factura", this::mostrarFacturas));
            menu.add(Box.createRigidArea(new Dimension(0, 14)));
        }
        if (usuario.getRol().equals("COCINA") || usuario.getRol().equals("ADMIN")) {
            menu.add(botonMenu("Monitor cocina", this::mostrarMonitorCocina));
            menu.add(Box.createRigidArea(new Dimension(0, 14)));
        }
        if (usuario.getRol().equals("ADMIN")) {
            JLabel admin = new JLabel("Solo disponible para admin");
            admin.setForeground(new Color(190, 190, 210));
            admin.setFont(new Font("SansSerif", Font.PLAIN, 11));
            admin.setAlignmentX(Component.LEFT_ALIGNMENT);
            menu.add(Box.createRigidArea(new Dimension(0, 10)));
            menu.add(admin);
            menu.add(Box.createRigidArea(new Dimension(0, 8)));
            menu.add(botonMenu("Registrar usuario", this::mostrarRegistrarUsuario));
            menu.add(Box.createRigidArea(new Dimension(0, 14)));
            menu.add(botonMenu("Registrar producto", () -> mostrarRegistrarProducto(false)));
            menu.add(Box.createRigidArea(new Dimension(0, 14)));
            menu.add(botonMenu("Registrar combo", () -> mostrarRegistrarProducto(true)));
        }
        menu.add(Box.createVerticalGlue());
        BotonRedondo salir = botonMenu("Cerrar sesión", () -> {
            usuario.logout();
    ...
    private BotonRedondo botonMenu(String texto, Runnable accion) {
        ...
        b.setBackground(new Color(30, 48, 125));
    ...
    private void setPantallaActual(Runnable pantalla) {
        this.pantallaActual = pantalla;
    }

    private void refrescarPantallaActual() {
        if (pantallaActual != null) pantallaActual.run();
    }

    private void limpiar(String titulo) {
        contenido.removeAll();
        tituloContenido.setText(titulo);
        contenido.add(tituloContenido, BorderLayout.NORTH);
    }

    private void finalizarPantalla(JComponent panel) {
    ...
    private void mostrarInicio() {
        ...
        JLabel label = new JLabel("Seleccione una opción.", SwingConstants.CENTER);
    ...
    private void mostrarCrearOrden() {
        ...
        JList<Producto> carrito = new JList<>(modeloCarrito);
        carrito.setFont(new Font("SansSerif", Font.PLAIN, 14));
        ...
        try {
            for (Producto p : BaseDatos.getProductos()) {
                ...
                item.addActionListener(e -> {
                    ...
                    actualizarTotal(modeloCarrito, total);
                });
                grid.add(item);
            }
        } catch (Exception ex) {
        ...
        derecha.setBorder(new EmptyBorder(15, 15, 15, 15));
        derecha.setBackground(new Color(235, 235, 235));
        JLabel carTitulo = new JLabel("Carrito", SwingConstants.CENTER);
        carTitulo.setFont(new Font("SansSerif", Font.BOLD, 24));
        derecha.add(carTitulo, BorderLayout.NORTH);
        derecha.add(new JScrollPane(carrito), BorderLayout.CENTER);
        JPanel acciones = new JPanel(new GridLayout(0, 1, 8, 8));
        acciones.setOpaque(false);
        JTextField cliente = new JTextField("Cliente final");
        BotonRedondo completar = new BotonRedondo("Completar orden", 14);
        completar.setBackground(new Color(73, 135, 36));
        BotonRedondo cancelar = new BotonRedondo("Cancelar orden", 14);
        cancelar.setBackground(new Color(150, 0, 10));
        acciones.add(cliente);
        acciones.add(total);
        ...
        completar.addActionListener(e -> {
            try {
                ...
                for (int i = 0; i < modeloCarrito.size(); i++) productos.add(modeloCarrito.get(i));
        ...
        finalizarPantalla(panel);
    }

    private void actualizarTotal(DefaultListModel<Producto> modelo, JLabel label) {
        double suma = 0;
        for (int i = 0; i < modelo.size(); i++) suma += modelo.get(i).calcularPrecio();
    ...
    private void mostrarOrdenes() {
        ...
        finalizarPantalla(new JScrollPane(tabla));
    }
    ...
    private void mostrarMonitorCocina() {
        ...
        try {
            for (Orden o : BaseDatos.getMonitorCocina().mostrarOrdenes()) {
                ...
                cambiar.setBackground(new Color(165, 205, 42));
                cambiar.setForeground(Color.BLACK);
                cambiar.addActionListener(e -> {
                    try {
                        BaseDatos.getMonitorCocina().actualizarEstadoOrden(o, o.getEstado().equals("PENDIENTE") ? "EN_PREPARACION" : "ENTREGADO");
                    } catch (Exception ex) {
                        mostrarError(ex);
                    }
                ...
                card.add(productos, BorderLayout.CENTER);
                card.add(cambiar, BorderLayout.SOUTH);
                panel.add(card);
            }
        } catch (Exception ex) {
            mostrarError(ex);
    ...
    private void mostrarFacturas() {
        ...
        try {
            for (Orden o : BaseDatos.getOrdenes()) modelo.addElement(o);
        } catch (Exception ex) {
            mostrarError(ex);
        }
        ...
        BotonRedondo generar = new BotonRedondo("Generar factura", 14);
        generar.setBackground(new Color(62, 111, 255));
        ...
        panel.add(new JScrollPane(factura), BorderLayout.CENTER);
    ...
    private void mostrarRegistrarUsuario() {
        ...
        JPanel form = formularioBase();
        JTextField nombre = new JTextField();
        JTextField user = new JTextField();
        JPasswordField pass = new JPasswordField();
        JComboBox<String> rol = new JComboBox<>(new String[]{"CAJERO", "COCINA", "ADMIN"});
        agregarCampo(form, "Nombre", nombre);
        agregarCampo(form, "Usuario", user);
        agregarCampo(form, "Contraseña", pass);
        agregarCampo(form, "Rol", rol);
        BotonRedondo guardar = new BotonRedondo("Guardar usuario", 14);
        guardar.setBackground(new Color(62, 111, 255));
        guardar.addActionListener(e -> {
            try {
                BaseDatos.registrarUsuario(nombre.getText(), user.getText(), new String(pass.getPassword()), (String) rol.getSelectedItem());
    ...
    private void mostrarRegistrarProducto(boolean combo) {
        ...
        JComboBox<String> tipo = new JComboBox<>(new String[]{"Hamburguesa", "Bebida", "Extra", "Combo"});
        tipo.setSelectedItem(combo ? "Combo" : "Hamburguesa");
        agregarCampo(form, "Nombre", nombre);
        agregarCampo(form, "Precio", precio);
        agregarCampo(form, "Tipo", tipo);
        BotonRedondo guardar = new BotonRedondo(combo ? "Guardar combo" : "Guardar producto", 14);
        guardar.setBackground(new Color(62, 111, 255));
        guardar.addActionListener(e -> {
            try {
                double p = Double.parseDouble(precio.getText());
                if (combo) BaseDatos.registrarCombo(nombre.getText(), p);
                else BaseDatos.registrarProducto(nombre.getText(), p, (String) tipo.getSelectedItem(), true);
                JOptionPane.showMessageDialog(this, combo ? "Combo registrado." : "Producto registrado.");
            } catch (Exception ex) {
                mostrarError(ex);
            }
        });
    ...
    private JPanel formularioBase() {
        ...
        form.setBorder(new EmptyBorder(30, 260, 120, 260));
